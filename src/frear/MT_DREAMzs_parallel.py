...
################################################################################
# Plot profile for fixed longitude
################################################################################
def plot_fixed_lon_profile(true_lon: float, true_lat: float, prior: Any, posterior: Any,
                       lower_bayes: np.ndarray, upper_bayes: np.ndarray,
                       settings: Dict[str, Any]) -> None:
    import matplotlib.pyplot as plt

    theta_init = prior['best'].copy()

    lat_index = settings['parnames'].index("lat")
    lon_index = settings['parnames'].index("lon")

    lats = np.linspace(lower_bayes[lat_index],
                       upper_bayes[lat_index], 200)

    logpost_vals = []

    for lat in lats:
        theta = theta_init.copy()
        theta[lon_index] = true_lon
        theta[lat_index] = lat

        val = posterior['density'](theta.reshape(1, -1))
        logpost_vals.append(val['posterior'][0])

    logpost_vals = np.array(logpost_vals)

    print("True latitude logpost:",
          logpost_vals[np.argmin(np.abs(lats - true_lat))])
    print("Max logpost:",
          np.max(logpost_vals))

    plt.figure()
    plt.plot(lats, logpost_vals)
    plt.axvline(true_lat, linestyle='--')
    plt.title("Log-posterior profile (lon fixed)")
    plt.xlabel("Latitude")
    plt.ylabel("Log posterior")
    plt.savefig(settings['logdir'] + "logpost_profile_fixed_lon.png")
    plt.show()
...
def initialize_worker(args: Dict[str, Any]) -> None:
    """Initialize worker for parallel processing
    
    Parameters:
        args (Dict[str, Any]): Dictionary containing necessary data for worker initialization
            - prior (Any): Prior distribution function
            - posterior (Any): Posterior distribution function
            - e (Any): Noise function for uniform distribution
            - eps (Any): Noise function for multinormal distribution
            - DEpairs (int): Number of pairs for Differential Evolution
            - zgammaFactor (float): Scale factor for gamma to control acceptance rate
            - npar (int): Number of parameters
            - nMTM (int): Number of proposals for Multiple Try Metropolis
            - CRupdatefreq (int): Frequency of updating crossover probabilities
            - pSnooker (float): probability of snooker update
            - X_shape (Tuple[int]): Shape of the shared memory array for X
            - X_name (str): Name of the shared memory for X
            - logpost_X_shape (Tuple[int]): Shape of the shared memory array for logpost_X
            - logpost_X_name (str): Name of the shared memory for logpost_X
            - Z_shape (Tuple[int]): Shape of the shared memory array for Z
            - Z_name (str): Name of the shared memory for Z
            - M_name (str): Name of the shared memory for M
            - CR_shape (Tuple[int]): Shape of the shared memory array for CR
            - CR_name (str): Name of the shared memory for CR
    """
    global _prior, _posterior, _e, _eps, _DEpairs, _zgammaFactor, _npar, _nMTM, _CRupdatefreq, _pSnooker
    _prior = args["prior"]
    _posterior = args["posterior"]
    _e = args["e"]
    _eps = args["eps"]
    _DEpairs = args["DEpairs"]
    _zgammaFactor = args["zgammaFactor"]
    _npar = args["npar"]
    _nMTM = args["nMTM"]
    _CRupdatefreq = args["CRupdatefreq"]
    _pSnooker = args["pSnooker"]

    global _X_memory, _logpost_X_memory, _Z_memory, _M_memory, _CR_memory
    _X_memory = shared_memory.SharedMemory(name=args['X_name'])
    _logpost_X_memory = shared_memory.SharedMemory(name=args['logpost_X_name'])
    _Z_memory = shared_memory.SharedMemory(name=args['Z_name'])
    _M_memory = shared_memory.SharedMemory(name=args['M_name'])
    _CR_memory = shared_memory.SharedMemory(name=args['CR_name'])

    global _X, _logpost_X, _Z, _M, _CR
    _X = np.ndarray(args['X_shape'], dtype=np.float64, buffer=_X_memory.buf)
    _logpost_X = np.ndarray(args['logpost_X_shape'], dtype=np.float64, buffer=_logpost_X_memory.buf)
    _Z = np.ndarray(args['Z_shape'], dtype=np.float64, buffer=_Z_memory.buf)
    _M = np.ndarray((1,), dtype=np.int64, buffer=_M_memory.buf)
    _CR = np.ndarray(args['CR_shape'], dtype=np.float64, buffer=_CR_memory.buf)


def update_chain_shared(ichain: int, iiter: int) -> Dict[str, Any]:
    """Update chain ichain one single step using shared memory for parallel processing
    ...
    global _X, _logpost_X, _Z, _M, _CR
    global _prior, _posterior, _e, _eps, _DEpairs, _zgammaFactor, _npar, _nMTM, _CRupdatefreq, _pSnooker
    update_data = update_chain(ichain=ichain, X=_X, logpost_X=_logpost_X, Z=_Z, M=_M[0],
                        CR=_CR, prior=_prior, posterior=_posterior,
                        e=_e, eps=_eps,
                        DEpairs=_DEpairs,
                        zgammaFactor=_zgammaFactor,
                        npar=_npar, nMTM=_nMTM,
                        iiter=iiter,
                        CRupdatefreq=_CRupdatefreq,
                        pSnooker=_pSnooker)
    _X[ichain, :] = update_data['X_new']
    lp = update_data['logpost_X_new']
    if isinstance(lp, dict):
        _logpost_X[ichain, 0] = lp['prior']
        _logpost_X[ichain, 1] = lp['likelihood']
        _logpost_X[ichain, 2] = lp['posterior']
    else:
        _logpost_X[ichain, :] = lp

    return {'ARs_new': update_data['ARs_new']}
...
def MT_DREAMzs(density: Any, sampler: Any, ll_logdensity: Any,
    ...
    # optional plot of latitude profile for fixed longitude - test
    if settings.get("plot_fixed_lon_profile", False):
        plot_fixed_lon_profile(true_lon=settings['trueValues'][settings['parnames'].index("lon")],
                               true_lat=settings['trueValues'][settings['parnames'].index("lat")],
                               prior=prior, posterior=posterior,
                               lower_bayes=lower_bayes, upper_bayes=upper_bayes,
                               settings=settings)

    # Ensure bounds are numpy arrays for proper comparison
    lower_bayes = np.asarray(lower_bayes)
    upper_bayes = np.asarray(upper_bayes)
    ...
    if X is None:
        X_shared_memory = shared_memory.SharedMemory(create = True, size=nchains * npar * float_size)
        X = np.ndarray((nchains, npar), dtype=np.float64, buffer=X_shared_memory.buf)
    ...
    logpost_X_shared_memory = shared_memory.SharedMemory(create = True, size=nchains * 3 * float_size)
    logpost_X = np.ndarray((nchains, 3), dtype=np.float64, buffer=logpost_X_shared_memory.buf)
    tmp = posterior['density'](X)
    logpost_X[:] = np.column_stack((tmp['prior'], tmp['likelihood'], tmp['posterior']))

    zchains[0, :, :] = np.column_stack((X, logpost_X)).T #(niter, npar+3, nchains)

    M_shared_memory = shared_memory.SharedMemory(create = True, size = 1 * np.dtype(np.int64).itemsize)
    M = np.ndarray((1,), dtype=np.int64, buffer=M_shared_memory.buf)
    if Zinit is None:
        M[0] = 10 * max(nchains, npar)  # N << M0 in laloy and vrugt 2012 #baytools: 10*npar
        Zinit = prior['sampler'](M[0])
    else:   
        M[0] = Zinit.shape[0]

    Z_shared_memory = shared_memory.SharedMemory(create = True, size = (M[0] + (niter // Zupdatefreq) * nchains) * npar * float_size)
    Z = np.ndarray((M[0] + (niter // Zupdatefreq) * nchains, npar), dtype=np.float64, buffer=Z_shared_memory.buf)
    Z[:] = np.full((M[0] + (niter // Zupdatefreq) * nchains, npar), np.nan)
    Z[:M[0], :] = Zinit
    ...
    CR_shared_memory = shared_memory.SharedMemory(create = True, size=nchains * CRupdatefreq * float_size)
    CR = np.ndarray((nchains, CRupdatefreq), dtype=np.float64, buffer=CR_shared_memory.buf)
    CR[:] = calcCR(nchains, CRupdatefreq, pCR)

    shared_objects = {
        'X': X_shared_memory,
        'logpost_X': logpost_X_shared_memory,
        'Z': Z_shared_memory,
        'M': M_shared_memory,
        'CR': CR_shared_memory
    }
    ...
    if settings['parallel']:
        init_args = {
            ...
            "pSnooker": pSnooker,
            "X_shape": X.shape,
            "X_name": X_shared_memory.name,
            "logpost_X_shape": logpost_X.shape,
            "logpost_X_name": logpost_X_shared_memory.name,
            "Z_shape": Z.shape,
            "Z_name": Z_shared_memory.name,
            "M_name": M_shared_memory.name,
            "CR_shape": CR.shape,
            "CR_name": CR_shared_memory.name
        }
        pool = Pool(processes=settings["nproc"], initializer=initialize_worker, initargs=(init_args,))
    ...
    for iiter in range(1, niter):
        ...
        if not settings["parallel"]:
            ...
            aux = pool.starmap(update_chain_shared, args)
