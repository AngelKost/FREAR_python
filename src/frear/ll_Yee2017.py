...
def dctrue(ctrue: np.ndarray, mod: np.ndarray, sigma_tot: np.ndarray, Y: np.ndarray, alpha: np.ndarray, beta: np.ndarray) -> np.ndarray:
    """Probability density for true concentration given modelled concentration
    
    Parameters:
        ctrue (np.ndarray): True concentration values
        mod (np.ndarray): Modelled concentration values
    ...
    Returns:
        pd_true (np.ndarray): Probability density values
    """
    coeff = Y / sigma_tot
    power = beta + 0.5
    dc = (ctrue - mod) ** 2
    denominator = (alpha + dc / (2 * sigma_tot ** 2)) ** power
...
def p_ndet_ctrue(ctrue: np.ndarray, L_C: np.ndarray, k_alpha: float = 1.645) -> np.ndarray:
    ...
    Returns:
        p_ndet_ctrue (np.ndarray): Probability of non-detection given true concentration
    """
...
def p_det_ctrue(ctrue: np.ndarray, L_C: np.ndarray, k_alpha: float = 1.645) -> np.ndarray:
    ...
    Parameters:
        ...
        k_alpha (float): Constant for detection limit calculation
    Returns:
        p_det_ctrue (np.ndarray): Probability of detection given true concentration
    """
    return 1.0 - p_ndet_ctrue(ctrue=ctrue, L_C=L_C, k_alpha=k_alpha)

def p_true_det_cmod(L_C: np.ndarray, mod: np.ndarray, sigma_tot: np.ndarray, Y: np.ndarray, 
    ...
    Returns:
        p_true_det_cmod (np.ndarray): Probability of a true detection given modelled concentration
    """
    if upper is None:
        upper = 20 * L_C
    integral, _ = integrate.quad(
        lambda ctrue: dctrue(ctrue, mod, sigma_tot, Y, alpha, beta),
        L_C, upper
    )
    return integral / zdctruenorm

def p_true_ndet_cmod(L_C: np.ndarray, mod: np.ndarray, sigma_tot: np.ndarray, Y: np.ndarray, 
                     alpha: np.ndarray, beta: np.ndarray, zdctruenorm: np.ndarray, upper: Optional[float] = None) -> np.ndarray:
    ...
    Parameters:
        ...
        upper (Optional[float]): Upper limit for integration
    Returns:
        p_true_ndet_cmod (np.ndarray): Probability of a true non-detection given modelled concentration
    """
    if upper is None:
        upper = 20 * L_C
    integral, _ = integrate.quad(
...
def p_cdet_true_ndet(cdet: np.ndarray, L_C: np.ndarray, k_alpha: float = 1.645) -> np.ndarray:
    """Probability of detection given a true non-detection (false alarm)
    ...
    Parameters:
        ...
        k_alpha (float): Constant for detection limit calculation
    Returns:
        p_cdet_true_ndet (np.ndarray): Probability of detection given a true non-detection concentration
    """
...
def p_ndet_cmod(L_C: np.ndarray, mod: np.ndarray, sigma_mod: np.ndarray, Y: np.ndarray, 
    ...
    Parameters:
        ...
        zdctruenorm (np.ndarray): Normalization factor for true concentration density
    Returns:
        p_ndet_cmod (np.ndarray): Probability of a non-detection given modelled concentration
    """
    MDC = 2 * L_C
    integral, _ = integrate.quad(
        lambda ctrue: p_ndet_ctrue(ctrue, L_C) * dctrue(ctrue, mod, sigma_mod, Y, alpha, beta) / zdctruenorm,
        0, 10 * MDC
    )
    return integral

def ll_Yee2017_det(ac: np.ndarray, alphas: np.ndarray = np.array(1.0 / np.pi), betas: np.ndarray = np.array(1.0),
                   Ys: Optional[np.ndarray] = None, ll_noRelease: Optional[np.ndarray] = None) -> np.ndarray:
    """Calculate log-likelihoods for detections according to Yee et al.(2017)
    
    Parameters:
        ac (np.ndarray): Array containing observed and modelled concentrations and uncertainties
            - obs (np.ndarray): Observed concentrations
            - mod (np.ndarray): Modelled concentrations
            - sigma_obs (np.ndarray): Observational uncertainties
            - sigma_mod (np.ndarray): Model uncertainties
            - MDC (np.ndarray): Minimum detectable concentrations
        alphas (np.ndarray): Parameter alpha
        betas (np.ndarray): Parameter beta
        Ys (Optional[np.ndarray]): Parameter Y
    ...
    if len(icalc) > 0:
        ...
        if len(icalc) > 0:
            ...
            for i in icalc:
                integral, _ = integrate.quad(
                    lambda ctrue: dctrue(ctrue, ac['mod'][i], sigma_tot[i], Ys[i], alphas[i], betas[i]),
                    lowLim[i],
                    uppLim[i]
                )
...
def ll_Yee2017_nondet(ac: Dict[str, Any], alphas: np.ndarray = np.array(1.0 / np.pi), betas: np.ndarray = np.array(1.0),
                      Ys: Optional[np.ndarray] = None, ll_noRelease: Optional[np.ndarray] = None) -> np.ndarray:
...
def getLikes_Yee2017(ac: Dict[str, Any], ll_out: Dict[str, Any] = {'alphas': 1. / np.pi,
                                                                'betas': 1,
                                                                'll_noRelease': None,
                                                                'Ys': None,
                                                                'll_factors': None}) -> Dict[str, Any]:
    """Calculate likelihoods according to Yee et al. (2017) for given AC data
    
    Parameters:
        ac (Dict[str, Any]): Array containing observed and modelled concentrations and uncertainties
            - obs (np.ndarray): Observed concentrations
            - mod (np.ndarray): Modelled concentrations
            - sigma_obs (np.ndarray): Observational uncertainties
            - sigma_mod (np.ndarray): Model uncertainties
            - MDC (np.ndarray): Minimum detectable concentrations
        ll_out (Dict[str, Any]): Dictionary containing parameters for likelihood calculation
            - alphas (np.ndarray): Parameter alpha
            - betas (np.ndarray): Parameter beta
            - ll_noRelease (np.ndarray): Precalculated log-likelihoods for no-release
            - Ys (np.ndarray): Parameter Y
            - ll_factors (np.ndarray): Likelihood factors
    Returns:
        result (Dict[str, Any]): Dictionary containing likelihood values
            - likes (np.ndarray): Log-likelihood values
    """
    ...
    if len(idet) != 0:
        likes[idet] = ll_Yee2017_det(ac=ac_det,
                                     alphas=alphas[idet],
                                     betas=betas[idet],
                                     Ys=Ys[idet],
                                     ll_noRelease=ll_noRelease[idet])
    if len(inondet) != 0:
        likes[inondet] = ll_Yee2017_nondet(ac=ac_nondet,
                                           alphas=alphas[inondet],
                                           betas=betas[inondet],
                                           Ys=Ys[inondet],
                                           ll_noRelease=ll_noRelease[inondet])
...
def preCalc_ll_noRelease(settings: Dict[str, Any], lower_bayes: np.ndarray, upper_bayes: np.ndarray,
    ...
    Parameters:
        ...
        ACfun_bayes (Any): Function to calculate AC values for Bayesian parameters
        ll_out (Dict[str, Any]): Dictionary containing parameters for likelihood calculation
            - alphas (np.ndarray): Parameter alpha
            - betas (np.ndarray): Parameter beta
            - ll_noRelease (np.ndarray): Precalculated log-likelihoods for no-release
            ...
            - ll_factors (np.ndarray): Likelihood factors
    Returns:
        ll_noRelease (np.ndarray): Precalculated log-likelihoods for no-release cases
    """
    param_noRelease = np.array([(u + l) / 2.0 for u, l in zip(upper_bayes, lower_bayes)])
    #param_noRelease[3] = -np.inf  # since Q <- 10^Q
    ...
    ll_noRelease = getLikes_Yee2017(ac_noRelease, ll_out)["likes"]
    return ll_noRelease
...
def preCalc_ll_factors(settings: Dict[str, Any], lower_bayes: np.ndarray, upper_bayes: np.ndarray,
                        ACfun_bayes: Any, ll_out: Dict[str, Any] = {'alphas': 1. / np.pi, 
                                                                    ...
                                                                    'll_noRelease': None,
                                                                    'Ys': None}) -> np.ndarray:
    """Precalculate likelihood factors for non-detection cases according to Yee et al.(2017)
    ...
    Parameters:
        ...
        ll_out (Dict[str, Any]): Dictionary containing parameters for likelihood calculation
            ...
            - ll_factors (np.ndarray): Likelihood factors
    Returns:
        ll_factors (np.ndarray): Precalculated likelihood factors for non-detection cases
    """
    ...
    for iobs in range(len(ac_dummy['obs'])):
        if ac_dummy['obs'][iobs] == 0:
            ...
            deltall1 = getLikes_Yee2017(ac1, ll_out)["likes"] - getLikes_Yee2017(ac2, ll_out)["likes"]
            deltall2 = getLikes_Yee2017(ac3, ll_out)["likes"] - getLikes_Yee2017(ac4, ll_out)["likes"]
